/* $Id: test_esign.C,v 1.9 2005/08/11 13:03:39 max Exp $ */
...
static u_int64_t tst_vtime;
static u_int64_t tst_stime;
...
#define COUNT 50000
    ...
    warn ("Signed %d msgs with %d bit key in %" U64F "u " 
	  TIME_LABEL " per signature\n", COUNT, sz, tst_stime / COUNT);
    warn ("Verified %d msgs with %d bit key in %" U64F "u " 
	  TIME_LABEL " per verify\n", COUNT, sz, tst_vtime / COUNT);
    ...
    warn ("Precomputed %d sigs with %d bit key in %" U64F "u " 
	  TIME_LABEL " per sig\n", COUNT, sz, tst_stime / COUNT);
    warn ("Did %d precomputed sigs with %d bit key in %"
