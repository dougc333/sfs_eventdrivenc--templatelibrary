dnl $Id: acsfs.m4,v 1.2 2006/05/31 16:30:36 max Exp $
dnl
...
dnl
dnl This doesn't work with later versions of sleepycat anyway
dnl
dnl for vers in $1; do
dnl     DB_DIR=`cd $srcdir && echo db-$vers.*/dist/`
dnl     if test -d "$srcdir/$DB_DIR"; then
dnl         DB_DIR=`echo $DB_DIR | sed -e 's!/$!!'`
dnl         break
dnl     else
dnl         unset DB_DIR
dnl     fi
...
AC_ARG_WITH(debug,
--with-debug="FLAGS"      Optimization and Debug flags for compiler,
DEBUG="$with_debug"
export DEBUG)
